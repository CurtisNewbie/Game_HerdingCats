...
public class HerdingCats {
	...
	public static void main(String[] args) {
		...
		// This is used to check whether the square entered for dog is illegitimate. If so, the program exists.
		 ...
		 * 
		 * Create 10 instances of class Cat and create new objects of Random class 
		 * to randomly generate numbers for cats' positions and cats' rules. The 
		 * rules determine how the cats move, ranging from 1 to 6. To ensure the 
		 * cats are created within the grid, the random numbers for positionX is ranging 
		 * from CAT_WIDTH to GRID_SIZE, and for positionY, it ranges from CAT_HEIGHT 
		 * to GRID_SIZE.
		 */
		Cat cat1 = new Cat(Cat.CAT_WIDTH + new Random().nextInt(GRID_SIZE - Cat.CAT_WIDTH +1), Cat.CAT_HEIGHT + new Random().nextInt(GRID_SIZE-Cat.CAT_HEIGHT+1), 1 + new Random().nextInt(MAX_RULE));
		Cat cat2 = new Cat(Cat.CAT_WIDTH + new Random().nextInt(GRID_SIZE - Cat.CAT_WIDTH +1), Cat.CAT_HEIGHT + new Random().nextInt(GRID_SIZE-Cat.CAT_HEIGHT), 1 + new Random().nextInt(MAX_RULE));
		Cat cat3 = new Cat(Cat.CAT_WIDTH + new Random().nextInt(GRID_SIZE - Cat.CAT_WIDTH +1), Cat.CAT_HEIGHT + new Random().nextInt(GRID_SIZE-Cat.CAT_HEIGHT), 1 + new Random().nextInt(MAX_RULE));
		Cat cat4 = new Cat(Cat.CAT_WIDTH + new Random().nextInt(GRID_SIZE - Cat.CAT_WIDTH +1), Cat.CAT_HEIGHT + new Random().nextInt(GRID_SIZE-Cat.CAT_HEIGHT), 1 + new Random().nextInt(MAX_RULE));
		Cat cat5 = new Cat(Cat.CAT_WIDTH + new Random().nextInt(GRID_SIZE - Cat.CAT_WIDTH +1), Cat.CAT_HEIGHT + new Random().nextInt(GRID_SIZE-Cat.CAT_HEIGHT), 1 + new Random().nextInt(MAX_RULE));
		Cat cat6 = new Cat(Cat.CAT_WIDTH + new Random().nextInt(GRID_SIZE - Cat.CAT_WIDTH +1), Cat.CAT_HEIGHT + new Random().nextInt(GRID_SIZE-Cat.CAT_HEIGHT), 1 + new Random().nextInt(MAX_RULE));
		Cat cat7 = new Cat(Cat.CAT_WIDTH + new Random().nextInt(GRID_SIZE - Cat.CAT_WIDTH +1), Cat.CAT_HEIGHT + new Random().nextInt(GRID_SIZE-Cat.CAT_HEIGHT), 1 + new Random().nextInt(MAX_RULE));
		Cat cat8 = new Cat(Cat.CAT_WIDTH + new Random().nextInt(GRID_SIZE - Cat.CAT_WIDTH +1), Cat.CAT_HEIGHT + new Random().nextInt(GRID_SIZE-Cat.CAT_HEIGHT), 1 + new Random().nextInt(MAX_RULE));
		Cat cat9 = new Cat(Cat.CAT_WIDTH + new Random().nextInt(GRID_SIZE - Cat.CAT_WIDTH +1), Cat.CAT_HEIGHT + new Random().nextInt(GRID_SIZE-Cat.CAT_HEIGHT), 1 + new Random().nextInt(MAX_RULE));
		Cat cat10 = new Cat(Cat.CAT_WIDTH + new Random().nextInt(GRID_SIZE - Cat.CAT_WIDTH +1), Cat.CAT_HEIGHT + new Random().nextInt(GRID_SIZE-Cat.CAT_HEIGHT), 1 + new Random().nextInt(MAX_RULE));
		...
		System.out.println("Please enter number to control the dog:(Number must be: 1-9)\nRemember dog can only move to adjacent sqaures.");
		System.out.println("When all the cats are herded into the same square you win.\n");
		
		// It's a loop that checks if the game wins and if the square number entered for dog is illegitimate.
		// As long as one of them (the boolean variables) becomes true, while loop won't be executed.
		while (!win && !illegalDogPosition) {
			...
			/* This moves the dog to the square entered. The move() method in Dog class returns false 
			 * when the number entered is ranging from 1 to 9, and is also not the adjacent squares of the
			 * current square. If the number entered >=1 and <=9 and is a adjacent square. The dog is successfully 
			 * moved. If this method returns false, this 'while' loop will run again until the number entered is a 
			 * legitimate and adjacent square. 
			 * 
			 * If the number entered is less than 1 and larger than 9, the dog moves to the illegitimate position.
			 * Then the loop below will skip the code; change the boolean variable - 'illegalDogPosition' for the 
			 * while loop above; displays "Existing!"; and 'terminates' the program.
			 */
			...
			/*
			 *  If the number entered for dog's square is larger than LARGEST_NUM_ENTER(9) and less than SMALLEST_NUM_ENTER(0), 
			 *  the program displays "Existing" and stops reacting. Probably because the object of the EasyGraphic is not 
			 *  closed, the console is not truly terminated, though the console has stopped responding. ()
			 */
			if (dog.getSquareDog() < SMALLEST_NUM_ENTER || dog.getSquareDog() > LARGEST_NUM_ENTER) {
				...
				/* Set dog positionX and positionY based on its square. Further, the use of methods - getCenterPositionX() and 
				 * getCenterPositionY() is because the dog only appears at the center of the square.
				 */
	...
	// Static method for drawing or redrawing grid. The grid must be 3*3.
	public static void drawGrid(EasyGraphics grid) {
		grid.clear();
		grid.moveTo(0, GRID_SIZE / 3);
		grid.lineTo(GRID_SIZE, GRID_SIZE / 3);
		grid.moveTo(0, (GRID_SIZE / 3) * 2);
		grid.lineTo(GRID_SIZE, (GRID_SIZE / 3) * 2);
		grid.moveTo(0, GRID_SIZE);
		grid.lineTo(GRID_SIZE, GRID_SIZE);
		grid.moveTo(GRID_SIZE / 3, 0);
		grid.lineTo(GRID_SIZE / 3, GRID_SIZE);
		grid.moveTo((GRID_SIZE / 3) * 2, 0);
		grid.lineTo((GRID_SIZE / 3) * 2, GRID_SIZE);
		grid.moveTo(GRID_SIZE, 0);
		grid.lineTo(GRID_SIZE, GRID_SIZE);
	}
	...
	public static int getCenterPositionsX(int square) {
		...
		default:
			return 0;
		// Because it has been verified in the main method that dog.getSqaureDog() can only range from 1-9.
